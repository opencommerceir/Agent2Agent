...
### Q11: Does the Agent Orchestrator itself publish MCP capabilities, or is it only ever a *caller* of every other module's capabilities?

🎯 **What the interviewer is REALLY testing:**
Understanding the Orchestrator's dual role — the platform's single biggest capability *consumer*, but also a deliberately narrow capability *publisher* — and why its own public surface stays small on purpose.

✅ **Model answer:**
"Both, but asymmetrically. As a consumer, it's the single largest capability caller on the entire platform — Stage 3 (question 4 of this file) can, in principle, invoke any of the other 127 capabilities across every business module while executing a Plan. As a publisher, though, its own MCP surface is deliberately narrow — `agent.goal.execute`, `agent.goal.get`, `agent.execution.list` cover it, not one capability per internal stage. The reason ties directly back to what kind of Context this module actually is (file 08, question 8): its Ubiquitous Language is about *how work gets done* (Goal, Plan, Execution), not *what work gets done* — publishing a separate capability for planning, or for tool-calling, or for reflection, would leak internal orchestration mechanics into the same public vocabulary Commerce/CRM/Finance use for real business actions, blurring exactly the distinction file 08 draws between a business Context and a coordination layer. Submitting a Goal and checking on it is the only shape an outside caller (a human, a dashboard, even a higher-level meta-agent) genuinely needs."

🔁 **Likely follow-ups:**
1. "Could Stage 5's delegation ever be triggered directly through MCP, bypassing a Goal?" → No — `DelegationRequest` is only ever created internally, as a real consequence of a Goal's own Stage-3 execution hitting a permission gap; there's no standalone 'create a delegation' capability, since that would let a caller manufacture a delegation with no real failed step behind it.
2. "Why 3 capabilities and not just 1?" → `agent.goal.execute` alone would force a caller into synchronous, blocking-until-done semantics for something that can genuinely take a while (an LLM round-trip, several capability calls); `.get`/`.list` exist so a caller can submit and poll, the same submit-then-poll shape file 09 (question 3) already covers for a queued Bulk Operation.

🚩 **Red flags:**
Assuming the Orchestrator must expose one capability per stage "for completeness" — missing that a narrow, deliberately-scoped public surface is the correct shape here, not an unfinished one.
