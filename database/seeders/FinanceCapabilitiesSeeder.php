...
use App\Modules\Finance\Interfaces\MCP\FinanceCapabilities;
use Illuminate\Database\Seeder;

/**
 * Registers the Finance module's capabilities into the Capability
 * Registry. Follows the same seeder pattern Demo/Commerce/
 * CRMCapabilitiesSeeder established (see DemoCapabilitiesSeeder's
 * docblock for why this cannot safely live in
 * FinanceServiceProvider::boot() instead — RefreshDatabase migrates
 * *after* providers boot).
 *
 * Idempotent: safe to run against a database that already has these rows.
 */
class FinanceCapabilitiesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach (FinanceCapabilities::definitions() as $definition) {
            try {
