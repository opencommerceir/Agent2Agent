...
use App\Modules\Workflows\Interfaces\MCP\WorkflowsCapabilities;
use Illuminate\Database\Seeder;

/**
 * Registers the Workflows module's capabilities into the Capability
 * Registry. Follows the same seeder pattern Demo/Commerce/CRM/
 * FinanceCapabilitiesSeeder established (see DemoCapabilitiesSeeder's
 * docblock for why this cannot safely live in
 * WorkflowsServiceProvider::boot() instead — RefreshDatabase migrates
 * *after* providers boot).
 *
 * Idempotent: safe to run against a database that already has these rows.
 */
class WorkflowsCapabilitiesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach (WorkflowsCapabilities::definitions() as $definition) {
            try {
