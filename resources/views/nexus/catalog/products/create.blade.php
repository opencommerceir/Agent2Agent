...
@section('title', t('messages.nexus.catalog.products.add'))

@section('content')
    <div class="mx-auto max-w-2xl">
        <x-nexus-panel :title="t('messages.nexus.catalog.products.add')">
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('nexus.catalog.products.store') }}" class="space-y-4">
                @csrf
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock_quantity" min="0" value="{{ old('stock_quantity', 0) }}" required class="w-full rounded-md border border-nexus-border bg-nexus-surface-1 px-3 py-2 text-sm text-nexus-text focus:border-nexus-cyan focus:outline-none">
                    </div>
                </div>
