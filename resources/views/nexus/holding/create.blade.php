...
@section('title', t('messages.nexus.holding.create_new'))

@section('content')
    <div class="mx-auto max-w-2xl">
        <x-nexus-panel :title="t('messages.nexus.holding.create_new')">
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('nexus.holding.store') }}" class="space-y-4">
                @csrf

                <div>
                    <label class="mb-1 block text-sm text-nexus-text">{{ t('messages.nexus.holding.form.name_fa') }}</label>
                ...
                <div>
                    <label class="mb-1 block text-sm text-nexus-text">{{ t('messages.nexus.holding.form.name_en') }}</label>
                ...
                <button type="submit" class="w-full rounded-md bg-nexus-cyan/20 px-4 py-2 text-sm font-semibold text-nexus-cyan hover:bg-nexus-cyan/30">
                    {{ t('messages.nexus.holding.form.submit') }}
                </button>
            </form>
        </x-nexus-panel>
    </div>
