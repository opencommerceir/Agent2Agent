...
<body class="min-h-screen bg-nexus-dark font-sans text-nexus-text antialiased" x-data="{ sidebarOpen: false }">
    <div class="flex min-h-screen flex-col">
        <header class="flex h-16 items-center justify-between border-b border-nexus-border px-4 sm:px-6">
            <div class="flex items-center gap-2 text-lg font-semibold nexus-glow-text">
                <span class="size-2 rounded-full bg-nexus-cyan animate-[breathe_2s_ease-in-out_infinite]"></span>
                Nexus
            </div>
            ...
            <div class="flex items-center gap-3">
                <div class="flex overflow-hidden rounded-md border border-nexus-border text-xs" x-data="{ language: '{{ dashboard_language()->value }}' }">
                    <a href="{{ route('language.switch', 'en') }}" @click="language = 'en'" :class="language === 'en' ? 'bg-nexus-cyan/20 text-nexus-cyan' : 'text-nexus-text-muted'" class="px-2 py-1">EN</a>
                    <a href="{{ route('language.switch', 'fa') }}" @click="language = 'fa'" :class="language === 'fa' ? 'bg-nexus-cyan/20 text-nexus-cyan' : 'text-nexus-text-muted'" class="px-2 py-1">FA</a>
                </div>
            </div>
        </header>
