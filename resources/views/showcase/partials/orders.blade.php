<div class="space-y-2">
    @forelse ($orders as $order)
        <div class="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 text-sm">
            <div class="min-w-0">
                <p class="truncate font-medium text-gray-800">{{ $order['orderNumber'] }}</p>
                <p class="text-xs text-gray-400">{{ ucfirst($order['status']) }}</p>
            </div>
            <span class="shrink-0 font-semibold text-gray-700">${{ number_format($order['totalAmount'] / 100, 2) }}</span>
        </div>
    @empty
