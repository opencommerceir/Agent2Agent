...
/**
 * Real Bearer token over POST /mcp/v1/execute, same rigor
 * ReviewCapabilityTest already applies for the manifest -> Seeder ->
 * CapabilityHandlerRegistry wiring, not just the Action in isolation
 * (already covered by GetBusinessAnalyticsActionTest).
 */
class BusinessAnalyticsCapabilityTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_businessAnalytics_viaMcp_returnsOwnData(): void
    {
        ...
        $response = $this->postJson('/mcp/v1/execute', [
            'capability' => 'nexus.analytics.business',
            'input' => [],
        ], ['Authorization' => "Bearer {$token}"]);

        $response->assertStatus(200);
        $this->assertEquals(0, $response->json('data.successRate'));
        $response->assertJsonPath('data.completedDeals', 0);
    }

    public function test_businessAnalytics_withoutPermission_isForbidden(): void
    {
        $business = $this->verifiedBusiness('Caller Co');
