...
class InviteCapabilityTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_inviteSend_viaMcp_recordsInviteAndChargesCredit(): void
    {
        ...
        app(GrantCreditsAction::class)->execute($caller->id, 100, CreditTransactionType::AdminGrant, 'test.seed');
        $token = $this->tokenFor($caller->id, ['nexus.growth.manage']);

        $response = $this->postJson('/mcp/v1/execute', [
            'capability' => 'nexus.invite.send',
            'input' => ['invitee_name' => 'Lead Co', 'invitee_email' => 'lead@example.com'],
        ], ['Authorization' => "Bearer {$token}"]);

        $response->assertStatus(200);
        $response->assertJsonPath('data.status', 'sent');
        $this->assertDatabaseHas('nexus_invites', ['inviter_business_id' => $caller->id, 'invitee_email' => 'lead@example.com']);
    }

    public function test_inviteSend_viaMcp_withoutPermission_isForbidden(): void
    {
        $caller = app(RegisterBusinessAction::class)->execute('من', 'Caller Co', BusinessType::Company, Industry::Technology);
        app(VerifyBusinessAction::class)->execute($caller->id);
